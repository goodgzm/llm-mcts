...
from cot_policy_pomdp import LLMAgent, LanguageNode
...
if __name__ == "__main__":
    ...
    run_name = f"{args.env_id}__task={args.exp_name}__stochastic={args.stochastic}__seed={args.seed}__{time_str}__llm={args.llm_base_model.split('/')[-1]}__normalization_mode={args.normalization_mode}__value_weight={args.value_weight}__path_num={args.path_num}__MCTS"
    if args.track:
        import wandb
    ...
    for path_num in range(0, args.path_num):
        ...
        root = LanguageNode(state=next_obs, task=args.env_id)
        agent.expand(next_obs, root, envs, root.get_history_prompt())
        done = next_done
        node = root
        ...
        # 探索路径 探索到 20 步或探索到任务完成条件终止
        while done[0].item() == 0 and step < args.depth:
            global_step += 1 * args.num_envs
            obs[step] = next_obs
            dones[step] = next_done

            action, next_node, action_name = agent.select(node)
            actions[step] = action
            ...
            if not done and next_node.is_leaf():
                ...
                agent.expand(next_obs, next_node, envs, next_node.get_history_prompt())
        ...
        # MC update
        print(f"global_step={global_step}, num_path = {path_num}, episodic_return={done[0].item()}, episodic_length={step}")
        writer.add_scalar("charts/episodic_return", done[0].item(), global_step)
