...
class LLMAgent(nn.Module):
    def __init__(self, normalization_mode = 'token', load_path = None, load_8bit = False, task = 3, device = None,
    ...
    def get_action_and_value(self, obs, action=None, is_warmup=False, return_value = True):
        ...
        # print("##########input_ids_size:", input_ids.size())
        # # input()
        attention_mask = inputs["attention_mask"].to(self.device)
        if is_warmup:
            with torch.no_grad():
            ...
            outputs = self.actor(input_ids, attention_mask=attention_mask)
        # print("##########outputs[logits]_size:", outputs.logits.size())
        # input()
        ...
        # print("##########action_list:", action_list)
        # print("##########action_list_size:", self.action_list.size())
        # print("##########action_list_ids:", self.action_list_ids)
        # print("##########action_list_ids_size:", self.action_list_ids.size())
        # input()
        ...
        # print("##########action_logits_reshape:", action_logits)
        # input()
        # todo 处理 NaN 数据
        assert not torch.isnan(action_logits).any(), "action_logits contains NaN values, but Categorical can't accept them"
        ...
        #probs.log_prob(action)：计算 log π(a|s)，用于 PPO 策略优化
        #probs.entropy()：计算 策略熵，用于 探索 vs. 开发
        #self.get_value(prompt)：获取 Critic 网络的价值估计 V(s)
