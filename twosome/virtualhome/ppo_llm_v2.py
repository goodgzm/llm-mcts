...
def parse_args():
    ...
    # env_parameter
    parser.add_argument('--env-id', action='store', type=str, default='VirtualHome-v2', help='Domain name')
    parser.add_argument('--debug', action='store', type=bool, default=False,
                        help='Whehter print the debug information and render')

    parser.add_argument('--load-8bit', action='store', type=bool, default=False,
                        help='Whether to convert model to 8bits')
    parser.add_argument('--save-path', action='store', type=str, default="saved_models",
    ...
    parser.add_argument('--resume', action='store', type=bool, default=False,
                        help='Whehter resume from previous checkpoint')
    parser.add_argument('--load-path', action='store', type=str, default="saved_models",
                        help='The path to load the checkpoint')
    parser.add_argument('--record-path', action='store', type=str, default="llm5_runs",
                        help='The path to save the tensorboard results')

    parser.add_argument('--normalization-mode', action='store', type=str, default="token",
                        help='The normalization mode of how to deal with the logits of each token')

    # todo add params
    parser.add_argument('--opt-num-cuda', action='store', type=int, default=0,
                        help='option a cuda to verify to all tensors to be on the same device')
    parser.add_argument('--llm-base-model', action='store', type=str, default="meta-llama/Llama-3.1-8B",
                        help='option a cuda to verify to all tensors to be on the same device')
    parser.add_argument('--llm-base-model-path', action='store', type=str, default=0,
                        help='option a cuda to verify to all tensors to be on the same device')
...
if __name__ == "__main__":
    ...
    policy_optimizer = optim.AdamW(filter(lambda p: p.requires_grad, agent.actor.parameters()),
                                   lr=args.policy_learning_rate, eps=1e-5, weight_decay=0)
    value_optimizer = optim.Adam(filter(lambda p: p.requires_grad, agent.critic.parameters()),
    ...
    for update in range(1, num_updates + 1 + num_critic_warm_up_updates):
        ...
        writer.add_scalar("losses/policy_update_times", policy_update_steps // args.gradient_checkpointing_steps,
                          global_step)
