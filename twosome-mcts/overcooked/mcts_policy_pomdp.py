...
class Node(object):
    ...
    def get_info(self):
        return {
            ...
            "value": self.value, # todo can delete value ???
...
class LanguageNode(Node):
    ...
    def ucb_score(self,):
        """
            calculate UCB score
        """
        return self.value if self._visit_count == 0 else math.sqrt(2 * math.log(self.parent._visit_count) / self._visit_count) + self.value / self._visit_count
...
class LLMAgent(nn.Module):
    ...
    def expand(self, obs, node, simulate_envs):
        # 将当前环境观测 obs 转换为文本数据
        ...
        for id, action in enumerate(action_list):
            ...
            child = LanguageNode(parent=node, state=next_obs, last_action=action,
                                 initial_value=probs.log_prob(id_tmp).exp())
