...
from rnd.rnd import RndRewardModel
...
class Node(object):
    ...
    def get_info(self):
        return {
            ...
            "value": self.value, # todo can delete?
    ...
    def to_json(self):
        ...
        for name, child_node in self.children.items():
            children[name] = child_node.to_json()

        rets = {"children": children, "info": self.get_info()}
        return rets
...
class LanguageNode(Node):
    ...
    def __init__(
        ...
        # 当前节点环境观测状态，注意是 state, 不是 state，是 vector 类型，因为 MCTS++ 一个节点可能能转移多种 state
        self.state = state,
        self.task = task
    ...
    def value(self,):
        """
            calculate value
        """
        return self._initial_value if self._visit_count == 0 else self._visit_count / self.parent._visit_count
...
class ActionNode(Node):
    """
        MCTS++'s action node：expand MCTS's action edge to MCTS++'s action node
    """
    # 当前节点环境观测状态文本描述
    text_state: Optional[str] = None
    # 当前节点执行的最后一个动作（智能体上一步执行的动作）
    ...
    def value(self, ):
        """
            modify value calculation
        """
        # version 1: origin
        # return self._initial_value if self._visit_count == 0 else (1 / self._initial_value * ((self._visit_count)**self._value_weight + 1)
        #                                                            + self._path_value / self._visit_count)

        # version 2: UCT
        return self._initial_value if self._visit_count == 0 else (self._value_weight * self._initial_value * math.sqrt(math.log(self.parent._visit_count) / self._visit_count)
                                                                    + self._path_value / self._visit_count)
        # version 3: UCC
        # return self._initial_value if self._visit_count == 0 else (self._initial_value / math.log(self._visit_count + 1)
        #                                                            + self._path_value / self._visit_count)
...
class LLMAgent:
    def __init__(self, task = 3, normalization_mode = "token", device = None, llm_base_model = "meta-llama/Llama-3.1-8B-Instruct", llm_base_model_path = "",
                 epsilon=0.1, alpha=0.99, gamma=0.99, rnd=False, tb_logger=None, value_weight = 0.5, rnd_weight = 0.5):
        super().__init__()
        self.task = task
        if task == 0:
            self.obs_shape =18
    ...
    def select(self, node):
        ...
        for action_tmp, child_tmp in node.children.items():
            ...
            # modify rnd and ucc value calculate
            if self.rnd:
                node_value = torch.tensor([child_tmp.UCC_value], device=self.device) # ucc value
            else:
                node_value = torch.tensor([child_tmp.value], device=self.device) # origin value
            assert(node_value.shape == torch.Size([1]))
            value_list.append(node_value)
            node_visit_count.append(child_tmp._visit_count + 1)
        # 将 value_list 和 node_visit_count 转换为张量
        values = torch.stack(value_list).view(-1)
        node_visit_count = torch.tensor(node_visit_count, device=self.device)
        if self.rnd:
            # 计算 RND 奖励，并将其与节点的标准值加权相加
            ...
            ex_reward = self._rng_weight * torch.div(ex_reward, node_visit_count)
        ...
        # todo:  whymodify 随机数小概率探索改成用 RND 来探索
        # if self.rnd:
        #     return torch.tensor([argmax]),values[argmax], node.children[action_list[argmax]],action_list[argmax]
        # if random.random() < self.epsilon:
        #     # 小概率进行探索
        #     sampled_index = torch.multinomial(values, 1).item()
        #     return torch.tensor([sampled_index]),values[sampled_index], node.children[action_list[sampled_index]],action_list[sampled_index]
        # else:
        #     return torch.tensor([argmax]), values[torch.argmax(values)], node.children[action_list[argmax]], action_list[argmax]
        #     return torch.tensor([argmax]), values[argmax], node.children[action_list[argmax]], action_list[argmax]
        return torch.tensor([argmax]), values[argmax], node.children[action_list[argmax]], action_list[argmax]

    def expand(self, obs, node, simulate_envs, if_action_node=False):
        # modify 如果是 动作节点，单独处理增加状态子结点
        if if_action_node:
            # 查询状态后的状态是否已经存在，若存在直接转移
            for key, node_tmp in node.children.items():
                ...
                if torch.equal(temp, obs_tmp):
                    ...
                    return node_tmp
            # 如果转移到的状态并补存在，创建一个新状态节点
            child = LanguageNode(state = obs, parent = node, initial_value = 1, last_action = last_action)
            node.children[str(len(node.children))] = child
            # todo why assert ??? --> carry bug
        ...
        # 使用 LLM tokenizer 将 sequence 转换为 token
        ...
        # modify build action child node and child's child next state node
        for id, action in enumerate(action_list):
            ...
            re = torch.tensor([reward], device=self.device).view(-1)
            child = ActionNode(parent=node, state=next_obs, last_action=action, initial_value=probs.log_prob(id_tmp).exp()+re, value_weight=self._value_weight)
            child_child = LanguageNode(parent=child, state=next_obs, last_action=action, initial_value = 1)
            # state node --> (action node) child --> (next state node) child's child
    ...
    def update(self, node, next_node, reward):
        """ 当前节点的路径价值根据与其下一个节点的奖励和路径价值差异进行更新 """
